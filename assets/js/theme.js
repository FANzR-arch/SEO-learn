...
(function(){
  ...
  function apply(t){
    ...
    if(m) m.setAttribute("content", t === "dark" ? "#0E131B" : "#1D5FBF");
  }
  ...
  function toggle(){
    ...
    if(S.redrawAll) S.redrawAll();   // SVG 组件的颜色取自 CSS 变量，换主题后重绘
  }
  ...
  if(window.matchMedia){
    ...
    var onSys = function(){ if(!stored()){ apply(systemDark() ? "dark" : "light"); syncButtons(); if(S.redrawAll) S.redrawAll(); } };
  ...
  S.themeToggleHTML = BTN_HTML;
  S.mountThemeToggle = mount;
  S.syncThemeButtons = syncButtons;
